...
O teorema de Minimax garante a existência de um valor $V$ e de uma estratégia para cada jogador de um jogo \emph{zero-sum}, de maneiraa que o primeiro jogador tenha um \emph{payoff} ótimo de $V$ e o segundo um \emph{payoff} ótimo de $-V$, dadas as estratégias dos seus oponentes.
...
\begin{algorithm}
\begin{algorithmic}
\Function{Minimax}{$Node$, $Player$} \Comment{Devolve o valor do melhor nó seguinte}
  ...
  \If{$Player=min$}
      ...
      \For{$son$ in \Call{Successors}{$Node$}}
          ...
          \State $m \gets$ \Call{Min}{$m$, $next$}
      \EndFor
  \ElsIf{$Player=max$}
      \State $m \gets -\infty$
      \For{$son$ in \Call{Successors}{$Node$}}
          \State $next \gets$ \Call{Minimax}{$son$, $min$}
          \State $m \gets$ \Call{Max}{$m$, $next$}
      \EndFor
  \EndIf
  \State\Return{m} 
\EndFunction
